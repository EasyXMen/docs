...
StbM_TimeStampExtendedType类型定义
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

+-----------+------------------------------------+--------------------+
| 名称      | StbM_TimeStampExtendedType         |                    |
+-----------+------------------------------------+--------------------+
| 类型      | Structure                          |                    |
+-----------+------------------------------------+--------------------+
| 成员      | StbM_TimeBaseStatusType::          | 时间基状态         |
...
+----+----------------------------------+-----------------------------+
|    | StbM_                            | 速率                        |
|    | RateDeviationType::RateDeviation | 偏差测量后直接计算速率偏差  |
+----+----------------------------------+-----------------------------+
...
StbM_GetCurrentTimeExtended函数定义
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

+-------------+---------------------------+---------------------------+
| 函数名称：  | St                        |                           |
|             | bM_GetCurrentTimeExtended |                           |
+-------------+---------------------------+---------------------------+
| 函数原型：  | FUNC(Std_ReturnType,      |                           |
|             | STBM_CODE)                |                           |
|             |                           |                           |
|             | Stb                       |                           |
|             | M_GetCurrentTimeExtended( |                           |
|             |                           |                           |
|             | StbM                      |                           |
...
| 否可重入：  |                           |                           |
+-------------+---------------------------+---------------------------+
| 输入参数：  | timeBaseId                | 时间基 ID                 |
+-------------+---------------------------+---------------------------+
| 输入        | 无                        |                           |
| 输出参数：  |                           |                           |
+-------------+---------------------------+---------------------------+
| 输出参数：  | timeStamp                 | 此时有效的当前时间戳      |
+-------------+---------------------------+---------------------------+
|             | userData                  | 时间基的用户数据          |
+-------------+---------------------------+---------------------------+
| 返回值：    | E_OK 成功                 |                           |
...
+----------------+-----------------------------------------------------+
| 函数名称：     | Rte_                                                |
|                | Call\_<UserModule>\_<UserPortName>_UpdateGlobalTime |
+----------------+-----------------------------------------------------+
| 运行           | 详见4.3.7                                           |
| 实体函数定义： |                                                     |
+----------------+-----------------------------------------------------+
...
Rte_Call\_<UserModule>\_<UserPort>_GetCurrentTimeExtended
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

+----------------+-----------------------------------------------------+
| 函数名称：     | Rte_Call\_                                          |
|                | <UserModule>\_<UserPortName>_GetCurrentTimeExtended |
+----------------+-----------------------------------------------------+
| 运行           | 详见4.3.4                                           |
| 实体函数定义： |                                                     |
+----------------+-----------------------------------------------------+
| 变体：         | Name=StbMSynchronizedTimeBase.SHORT-NAME            |
+----------------+-----------------------------------------------------+
| 生成条件：     | 1. StbMSynchronizedTimeBaseIdentifier < 128         |
|                |                                                     |
|                | And                                                 |
|                |                                                     |
|                | 2. StbMGeneral/StbMGetCurrentTimeExtendedAvailable  |
|                |    == true                                          |
+----------------+-----------------------------------------------------+
| 端口类型：     | Provided Port                                       |
+----------------+-----------------------------------------------------+
...
.. _rte_call_usermodule_userport_getcurrenttimeextended-1:
...
+----------------+-----------------------------------------------------+
| 函数名称：     | Rte_                                                |
|                | Call\_<UserModule>\_<UserPortName>_GetRateDeviation |
+----------------+-----------------------------------------------------+
| 运行           | 详见4.3.13                                          |
| 实体函数定义： |                                                     |
+----------------+-----------------------------------------------------+
...
图5-1 StbMGeneral工具配置

表5-2 StbMGeneral配置描述
...
|                     | 依赖关系  | 无          |    |       |   |      |
+---------------------+-----------+-------------+----+-------+---+------+
| StbMGetCurrentTi    | 取值范围  | True、False |    | 默认  |   | F    |
| meExtendedAvailable |           |             |    | 取值  |   | alse |
+---------------------+-----------+-------------+----+-------+---+------+
|                     | 参数描述  | 是否支持    |    |       |   |      |
|                     |           | 获取64位的  |    |       |   |      |
|                     |           | 扩展形式的  |    |       |   |      |
...
图5-2 StbMSynchronizedTimeBase工具配置

表5-3 StbMSynchronizedTimeBase配置描述
...
:mark:`\*注：如果打开TimeCorrection相关功能，则需要硬件支持double/float的数据类型，因为相关计算会使用。`

|image5|

图5-3 StbMTimeCorrection工具配置

表5-4 StbMTimeCorrection配置描述

+--------------------+-----------+------------------+---+----------+---+---+
| **UI名称**         | **描述**  |                  |   |          |   |   |
+--------------------+-----------+------------------+---+----------+---+---+
| StbMAllowMa        | 取值范围  | True、False      | 默 |         |   | 无 |
| sterRateCorrection |           |                  | 认 |         |   |   |
|                    |           |                  | 取 |         |   |   |
|                    |           |                  | 值 |         |   |   |
+--------------------+-----------+------------------+---+----------+---+---+
|                    | 参数描述  | 作为时           |   |          |   |   |
...
图5-4 StbMLocalTimeClock工具配置

表5-5 StbMLocalTimeClock配置描述

+-------------------+-----------+------------------+---+----------+---+-----+
| **UI名称**        | **描述**  |                  |   |          |   |     |
+-------------------+-----------+------------------+---+----------+---+-----+
| S                 | 取值范围  | 0 .. 4294967295  | 默 |         |   | 无  |
| tbMClockFrequency |           |                  | 认 |         |   |     |
|                   |           |                  | 取 |         |   |     |
|                   |           |                  | 值 |         |   |     |
+-------------------+-----------+------------------+---+----------+---+-----+
...
图5-5 StbMTimeRecording工具配置

表5-6 StbMTimeRecording配置描述
...
|                   | 依赖关系  | 无               |   |          |   |     |
+-------------------+-----------+------------------+---+----------+---+-----+
| StbMSyncTimeRe    | 取值范围  | Function         | 默 |         |   | 无  |
| cordBlockCallback |           |                  | 认 |         |   |     |
|                   |           |                  | 取 |         |   |     |
|                   |           |                  | 值 |         |   |     |
+-------------------+-----------+------------------+---+----------+---+-----+
...
图5-6 StbMNotificationCustomer工具配置

表5-7 StbMNotificationCustomer配置描述

+-------------------+-----------+------------------+---+----------+---+-----+
| **UI名称**        | **描述**  |                  |   |          |   |     |
+-------------------+-----------+------------------+---+----------+---+-----+
| StbMNotif         | 取值范围  | 0 .. 65535       | 默 |         |   | 无  |
| icationCustomerId |           |                  | 认 |         |   |     |
|                   |           |                  | 取 |         |   |     |
|                   |           |                  | 值 |         |   |     |
+-------------------+-----------+------------------+---+----------+---+-----+
...
图5-7 StbMTriggeredCustomer工具配置

表5-8 StbMTriggeredCustomer配置描述

+--------------------+----------+------------------+---+----------+---+-----+
| **UI名称**         | **描述** |                  |   |          |   |     |
+--------------------+----------+------------------+---+----------+---+-----+
| StbMTrigg          | 取值范围 | 0 .. 4294967295  | 默 |         |   | 无  |
| eredCustomerPeriod |          |                  | 认 |         |   |     |
|                    |          |                  | 取 |         |   |     |
|                    |          |                  | 值 |         |   |     |
+--------------------+----------+------------------+---+----------+---+-----+
|                    | 参数描述 | 触发用户的触发   |   |          |   |     |
