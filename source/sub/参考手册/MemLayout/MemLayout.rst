...
Meanwhile, Sections can be referenced by the MemMap module to provide it with linking syntax for the Sections.
...
- 链接文件：需要拷贝到工程的主链接文件，或者被其包含
  ...
  Link file: Needs to be copied to the project's main link file or included by it
...
MemLayout is a non-standard AUTOSAR module designed to bridge the MemMap module and the compiler. It manages the chip's RAM and ROM and generates corresponding link files.
...
MemLayout can synchronize with the EcuC module to automatically create chip memory areas.
...
MemLayout only creates commonly used types of HwMemory. Users need to manually supplement the Name property for each HwMemory.
...
Special memory areas in the system require manual creation of MemoryType and then association with the corresponding SectionGroup.
...
Select the ORIENTAIS configuration tool MemLayout module, choose the Synchronize Module option in the MemLayout module, and generate a configuration file;

打开生成的配置文件，补充每个HwMemory的Name属性，创建特殊的MemoryType并关联SectionGroup；

Open the generated configuration file, supplement the Name property for each HwMemory, create special MemoryTypes and associate SectionGroups;
...
Link files for each platform need to be manually copied into the project.
...
It is necessary to manually name the Name property of each HwMemory; the name comes from the memory names in the main link file, such as flash0, dsram0, etc.
...
If there is no corresponding memory in the main link file, multiple HwMemory instances can be set to the same name. For example, if a chip only has global SRAM and no SRAM for each core, you can consider setting the HwMemory of each core to the name of the global SRAM.
...
Special memory regions in the system require manual creation of MemoryType and then association with the corresponding SectionGroup.
