...
.. list-table::
   ...
   * - SW-ADDR-METHOD
     - 用于为数据或代码对象分配公共的地址方法
       ...
       A common address method used to allocate data or code objects

   * - MEMORY-SECTION
     - 提供Implementation中用于代码或数据的抽象内存部分的描述。它应该由Bsw或Swc的Implementation创建
       
       Provides a description of abstract memory sections used for code or data in the Implementation. It should be created by the Implementation of Bsw or Swc

   * - 名称推导映射（Name Derivation Mapping）
     - 当配置选项AutoMappingByShortName==True时，MemMap模块根据MEMORY-SECTION的名称进行推导映射
       
       When the configuration option AutoMappingByShortName == True, the MemMap module performs derivation mapping based on the name of MEMORY-SECTION

   * - 手动映射（Manual Mapping）
     - 当配置选项EnableManual==True时，所有模块的相同类型的MEMORY-SECTION映射到同一个抽象的MAKW,并集中放置在MemMap.h文件中,允许修改此文件进行手动映射
       
       When the configuration option EnableManual == True, MEMORY-SECTIONs of the same type from all modules are mapped to the same abstract MAKW and centrally placed in the MemMap.h file, allowing modification of this file for manual mapping

   * - 工具配置映射（Tool Configuration Mapping）
     - 通过MemMapGenericMapping对SW-ADDR-METHOD进行映射或通过MemMapSectionSpecificMapping对MEMORY-SECTION进行映射
       
       Mapping SW-ADDR-METHOD through MemMapGenericMapping or mapping MEMORY-SECTION through MemMapSectionSpecificMapping

   * - 默认映射（Default Mapping）
     - 对未使用“工具配置映射”，“名称推导映射”和“手动映射”的MEMORY-SECTION，使用其引用的SW-ADDR-METHOD的映射规则。SW-ADDR-METHOD只能通过工具配置映射，工具会为默认的SW-ADDR-METHOD创建映射规则，自定义的SW-ADDR-METHOD则需要用户通过配置工具映射
       
       For MEMORY-SECTIONs that do not use "Tool Configuration Mapping", "Name Derivation Mapping" and "Manual Mapping", the mapping rules of the referenced SW-ADDR-METHOD are used. SW-ADDR-METHOD can only be mapped through tool configuration. The tool will create mapping rules for the default SW-ADDR-METHOD, while custom SW-ADDR-METHODs require users to map through the configuration tool
...
The MemMap module is the core hub of AUTOSAR memory management, which achieves:

 #. 精准控制代码/数据的内存分配；

    Accurately control the memory allocation of code/data;

 #. 提升性能（快速内存）与安全性（ASIL 隔离）；

    Improve performance (fast memory) and security (ASIL isolation);

 #. 确保跨编译器/平台的可移植性；

    Ensure cross compiler/platform portability;

 #. 为链接器提供标准化输入，完成物理地址映射。

    Provide standardized input for the linker and complete physical address mapping.
...
The MemMap module has been extended from user convenience, to support the following requirements:
...
This function provides a mapping method based on the MEMORY-SECTION name, which maps all MEMORY-SECTIONs in the system.
...
When this function is enabled, it means that the tool does not need to generate link statements. In addition to generating the MemMap.h file for the module, an additional MemMap.h file will be generated. Users can complete memory mapping by modifying these files on their own.
...
Open the generated configuration file, select the mapping rule in MemMapGeneration, and generate the memory mapping header file for each module;
...
Integrate the memory mapping header file, configuration file, and other source files;
...
The memory mapping header files for each module, for example: the MemMap. h file for the OS module is Os_MemMap. h.
...
The priority relationship among them is as follows:
Tool configuration mapping (needs configuration) > Name derivation mapping (needs enabling) > Default mapping (needs association) = Manual mapping (needs enabling)
...
.. figure:: ../../../_static/参考手册/MemMap/MemorySection映射逻辑.png
   :name: MemorySection映射逻辑
   :align: center

   MemorySection映射逻辑

   MemorySection Mapping Logic

.. figure:: ../../../_static/参考手册/MemMap/SwAddrMethod映射逻辑.png
   :name: SwAddrMethod映射逻辑
   :align: center

   SwAddrMethod映射逻辑

   SwAddrMethod Mapping Logic
...
Tool configuration mapping is a standard mapping method compliant with the AUTOSAR MemMap tool, which requires users to implement mapping through:

- MemMapSectionSpecificMapping容器关联MEMORY-SECTION和MemMapAddressingModeSet

  The MemMapSectionSpecificMapping container to associate MEMORY-SECTION with MemMapAddressingModeSet

- 或通过MemMapGenericMapping容器关联SwAddrMehtod和MemMapAddressingModeSet实现映射

  Or the MemMapGenericMapping container to associate SwAddrMethod with MemMapAddressingModeSet
...
.. figure:: ../../../_static/参考手册/MemMap/MemMapGenericMappingg.png
   ...
   MemMapGenericMapping is mapped as SW-ADDR-METHOD mapping.
...
.. caution:: 注意（Note）
  ...
  This method has the highest priority, which means that once MEMORY-SECTION or SW-ADDR-METHOD is configured through this method, it will not be affected by other mapping rules.
...
MemMapAddressingModeSet is a collection of MemMapAddressingMode, which contains one or more MemMapAddressingMode.

- MemMapSupportedSectionType表示MemMapAddressingModeSet下的MemMapAddressingMode支持那种类型的Section，类型不符合的映射会导致工具报错

  MemMapSupportedSectionType indicates the type of Section supported by MemMapAddressingMode under MemMapAddressingModeSet. Mappings with inconsistent types will cause tool errors.
...
- MemMapSupportedSectionInitializationPolicy参数只允许在MemMapSupportedSectionType为MEMMAP_SECTION_TYPE_VAR时可选，标志支持的初始化类型（INIT，CLEARED，POWER-ON-CLEARED）
  ...
  The MemMapSupportedSectionInitializationPolicy parameter is only optional when MemMapSupportedSectionType is MEMMAP_SECTION_TYPE_VAR, which marks the supported initialization types (INIT, CLEARED, POWER-ON-CLEARED).
...
MemMapAddressingMode represents a mapping rule, which may be implemented by a section of #pragma statements in actual code.

MemMapAddressingModeStart和MemMapAddressingModeStop表示#pragma语句的具体内容，例如

MemMapAddressingModeStart and MemMapAddressingModeStop represent the specific content of the #pragma statements, for example.
...
If you do not wish to perform mapping manually, you can reference a MemLayoutRegion. The tool will deduce the actual #pragma statements based on the properties of the MemLayoutRegion. For the calculation method, please refer to the MemLayout User Manual.

.. caution:: 注意（Note）

   MemLayoutRegionRef的配置优先级高于MemMapAddressingModeStart和MemMapAddressingModeStop，这意味着引用MemLayoutRegion之后，手动填写的映射规则将被工具忽略

   The configuration priority of MemLayoutRegionRef is higher than that of MemMapAddressingModeStart and MemMapAddressingModeStop. This means that after referencing a MemLayoutRegion, the manually filled mapping rules will be ignored by the tool.
...
This function provides a mapping method based on the name of MEMORY-SECTION, which performs memory mapping for all MEMORY-SECTIONs in the system. It needs to be enabled through the AutoMappingByShortName switch.
...
Default mapping will map MEMORY-SECTIONs that are not configured with :ref:工具配置映射<工具配置映射> and not matched by :ref:名称推导映射<名称推导映射>.
...
This MEMORY-SECTION will be mapped according to the mapping rules of the SW-ADDR-METHOD (through MemMapGenericMapping).
...
The MemMap module provides a one-click mapping creation function for the SW-ADDR-METHODs that exist by default in the system. All mapping functions can be created immediately by clicking the Synchronize Modules button of the MemMap module.
...
The objects created by this function will reference the containers of another module (MemLayout). Therefore, the prerequisite for using it is that the MemLayout module has first used the Synchronize Modules function. See also the MemLayout User Manual [link].
...
.. figure:: ../../../_static/参考手册/MemMap/SynchronizeMemMap_MemMapAddressingModeSet.png
   ...
   MemMapAddressingModeSet Created after MemMap Synchronize
...
.. figure:: ../../../_static/参考手册/MemMap/SynchronizeMemMap_MemMapGenericMapping.png
   ...
   MemMapGenericMapping Created after MemMap Synchronize
...
.. caution:: 注意（Note）
   ...
   The SW-ADDR-METHOD configured through the SWC tool belongs to the custom scope and cannot use the default mapping. It is required to use :ref:`工具配置映射<工具配置映射>`.
...
Checking the EnableManual switch indicates that the manual mapping mode is activated.
