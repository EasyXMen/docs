...
.. list-table::
   ...
   * - CanNm.h
     - CanNm模块头文件，包含API函数的扩展声明。(CanNm module header file, containing extended declarations of API functions)

   * - CanNm.c
     - CanNm模块源文件，包含了API函数的实现。(CanNm module source file, containing the implementation of API functions)

   * - CanNm_Type.h
     - CanNm数据类型定义文件，定义了配置和内部数据结构。(CanNm data type definition file, defining configuration and internal data structures)

   * - CanNm_Internal.h
     - CanNm内部数据类型定义文件，定义了部分宏定义，内部使用的变量扩展声明。(CanNm internal data type definition file, defining macros and extended declarations of internally used variables)
...
.. list-table::
   ...
   * - CanNm_Cfg.h
     - 定义CanNm模块预编译时用到的配置参数。(Defines configuration parameters used during CanNm module pre-compilation)

   * - CanNm_Cfg.c
     - 定义CanNm模块生成的配置参数。(Defines configuration parameters generated by the CanNm module)

   * - CanNm_PBcfg.c
     - 定义CanNm模块向外部模块提供的配置参数。(Defines configuration parameters provided by the CanNm module to external modules)
